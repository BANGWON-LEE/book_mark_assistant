...
export const Popup = () => {
  ...
  const updateBookMarks = async (bookmark, newIndex) => {
    ...
    console.log('클리어 최종 : ', bookmark, newIndex)
  ...
  const searchBookmarks = async (bookMarkWord) => {
    ...
    await chrome.bookmarks.search(bookMarkWord, function (results) {
      ...
      if (results.length > 0) {
        ...
        results
          .filter((id) => !fixedIdArray.includes(id))
          .map((bookmark, index) => {
  ...
  const [uiActionBtn, setUiActionBtn] = useState(false)
  const clearBookMark = (bookmarkId) => {
    ...
    console.log('클리어4', fixedCnt)
    updateBookMarks({ id: bookmarkId }, Number(fixedCnt))
  ...
  const handleKeyDown = (event) => {
    console.log('엔터')
    if (event.key === 'Enter') {
      setBookMarkSearchWord() // Enter 키를 누르면 클릭 이벤트 실행
    }
  }
    ...
    <div className="book_mark_box">
      ...
      <div className="book_mark_box_inner">
        <ul className="book_mark_box_text_ul">
          {bookmarks.length === 0 ? (
            <p className="text_not_found">No bookmarks found.</p>
          ) : (
